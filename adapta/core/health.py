...
class HealthMonitor:
    ...
    async def check_chroma(self) -> HealthCheck:
        start = time.time()
        try:
            from adapta.services.rag import _get_chroma_client

            _get_chroma_client().heartbeat()
            return HealthCheck(
                name="chroma",
                status=HealthStatus.HEALTHY,
                message="Chroma reachable",
                duration_ms=(time.time() - start) * 1000,
            )
        except Exception as exc:
    ...
    async def run_all_checks(self) -> SystemHealth:
        ...
        for c in checks:
            if isinstance(c, HealthCheck):
                ...
                valid.append(
                    HealthCheck(
                        name="unknown", status=HealthStatus.UNHEALTHY, message="Check crashed"
                    )
                )
