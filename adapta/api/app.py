...
def create_app() -> FastAPI:
    ...
    async def deep_health():
        from adapta.core.health import get_health_monitor
    ...
    if settings.metrics_enabled:
        ...
        async def metrics():
            ...
            mc = get_metrics_collector()
            try:
                from adapta.services.jobs import QUEUE_KEY, get_job_queue
    ...
    async def gpu_info():
        from adapta.core.gpu import get_gpu_config
    ...
    # ---------------------------------------------------------------------------
    # Operator console — static SPA (Vite+Svelte, built to adapta/console/dist/).
    ...
    if _console_dist.is_dir() and any(_console_dist.iterdir()):
        ...
        logger.warning(
            "Operator console dist not present at %s — UI unavailable (run `npm run build` in adapta/console/)",
            _console_dist,
        )
