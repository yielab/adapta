...
Runs from the entrypoint after migrations (``python -m adapta.db.seed``). Gated by
``ADAPTA_SEED_DEFAULT_ADMIN`` (the compose file enables it) and idempotent: it only
...
from adapta.db.models import Org, Role, Team, TeamMember
from adapta.db.session import AsyncSessionLocal, engine
from adapta.services.auth import create_user

logger = logging.getLogger("adapta.seed")


async def seed_default_admin(session_factory=AsyncSessionLocal) -> bool:
    """Create org/team/admin from the ADAPTA_DEFAULT_ADMIN_* settings if the DB is
...
async def _main() -> None:
    if not settings.seed_default_admin:
        logger.info("Seed disabled (ADAPTA_SEED_DEFAULT_ADMIN is off).")
        return
    try:
