...
| **Image fine-tunes (§V)** | 🔨 **V0–V4 done** (2026-06-11) — spikes, contracts (V1), data plane (V2), worker training/eval/conversion (V3), and **serving** (V4: mmproj chat-handler load, OpenAI image content-parts, image context-fit) shipped. GPU e2e green end to end incl. the served leg (`test_vlm_lora_e2e.py`: held-out score 1.000 vs base 0.011; the served answer for an unseen emblem is the trained association); contract gate 1485/1485 zero 5xx over the new surface. **Next: §V5 console** (bundle upload UI, Playground image attach, user-guide docs). |
