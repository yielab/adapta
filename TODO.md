...
#### V0.3 `[BE]` Spike: VLM QLoRA → GGUF LoRA → served adapter ("visual Quoria") — ◐ partially de-risked (2026-06-10)

- [x] **Converter finding.** The vendored `b4576` `convert_hf_to_gguf.py` registers `Qwen2VLForConditionalGeneration` but **not** `Qwen2_5_VLForConditionalGeneration`; tag `b5170` registers both (verified upstream), and `convert_lora_to_gguf.py` resolves through that same registry. → **Pin bump to ≥ b5170 required** for Qwen2.5-VL LoRA conversion. The bump touches the *production* text-conversion path too, so it must re-run `test_lora_e2e.py` as its regression gate.
- [x] **Trainability preflight.** Worker has transformers 5.11.0 (`Qwen2_5_VLForConditionalGeneration` present), torch 2.12 cu130, CUDA available, peft 0.19.1. **Gap: `pillow` is not in the worker image** (the VLM processor needs it) — per constraint #2 it must be added to `[training]` extras + `make up` rebuild before the training half of this spike can run (this is V3.1's first item, pulled forward).
- [ ] **Remaining.** (1) Add `pillow` to `[training]`; bump the vendored llama.cpp pin to ≥ b5170 (Dockerfile) + rebuild worker; re-run `test_lora_e2e.py` (text regression at the new pin). (2) QLoRA Qwen2.5-VL-3B on ~30 synthetic pairs (image of X → invented word), **vision tower frozen, LM projection modules only**. (3) Convert adapter → `.gguf`; serve base+mmproj+`lora_path`; assert the invented word appears with the image and not from the base.
- [ ] **Acceptance.** Invented-word assertion passes on the live GPU. **If conversion or lora+mmproj loading fails after reasonable effort: STOP** and re-decide with the fallback on the table (Strategy B: transformers-based GPU serving for vision endpoints — a much larger lift; likely "defer feature").
