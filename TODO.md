...
5. `[x]` **Document host prerequisites** (2026-06-09). README "Enabling the GPU (fine-tuning only)" section documents the one-time host steps (NVIDIA driver → `nvidia-container-toolkit` → `nvidia-ctk runtime configure` → `docker run --gpus all` sanity check), the opt-in overlay command, and the `grep "GPU ready"` check (matches the worker's startup banner). VRAM table lives in OPERATIONS §6 (ties to §3.3/§4.5); the optional `nvidia/cuda:*-runtime` base is still flagged in `Dockerfile`.
