...
### 4.2b Make GPU/CUDA training actually work (P0 — pairs with §4.2) — 🟡 image verified + all code/docs done; only the host-toolkit install (step 3) remains
...
2. `[x]` **GPU is the DEFAULT; CPU is the opt-out** (reworked 2026-06-09 per the product intent that this is a GPU product). The `worker` reserves the host GPU directly in `docker-compose.yml` (`deploy.resources.reservations.devices`, `driver: nvidia, count: all`), so a plain `docker compose up` uses the GPU. A CPU-only host layers `docker-compose.cpu.yml` (`-f docker-compose.yml -f docker-compose.cpu.yml`), which uses the Compose **`!reset`** tag to clear the reservation — necessary because a normal override **can't subtract** a list item (Compose merges/keeps the base device; verified via `docker compose config`). The old opt-in `docker-compose.gpu.yml` was deleted. `nvidia-ctk runtime configure` is run **without** `--set-as-default`, so GPU access stays scoped to `brain-worker` and no other container/project is affected. All three merges (base / base+dev-override / base+cpu) verified via `docker compose config`.
...
5. `[x]` **Document host prerequisites** (2026-06-09). README "GPU" section documents the one-time host steps (NVIDIA driver → `nvidia-container-toolkit` → `nvidia-ctk runtime configure` **without** `--set-as-default` → `docker run --gpus all` sanity check), that GPU is the default + the CPU opt-out, and the `grep "GPU ready"` check (matches the worker's startup banner). VRAM table lives in OPERATIONS §6 (ties to §3.3/§4.5); the optional `nvidia/cuda:*-runtime` base is still flagged in `Dockerfile`.
