...
| **Image fine-tunes (§V)** | 🔨 **V0–V5 done** (2026-06-11) — spikes, contracts (V1), data plane (V2), worker training/eval/conversion (V3), **serving** (V4: mmproj chat-handler load, OpenAI image content-parts, image context-fit), and **console + docs** (V5: `GET /v1/models` catalog endpoint, modality-aware create/fine-tune flows, Playground image attach, user-guide vision section). GPU e2e green end to end incl. the served leg (`test_vlm_lora_e2e.py`: held-out score 1.000 vs base 0.011; the served answer for an unseen emblem is the trained association); contract gate 1458/1458 zero 5xx incl. the new op. **Next: §V6 hardening** (exit criteria: CI integration, final snapshot). |
