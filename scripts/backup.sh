#!/usr/bin/env bash
# Adapta — backup (A4.12). Automates docs/reference/OPERATIONS.md §2.
#
# Backs up, from the same window (so a registered adapter row always has its file):
#   - Postgres (schema + data)         -> backup/adapta-<date>.sql.gz
...
# Cron:    0 3 * * *  cd /opt/adapta && scripts/backup.sh >> backup/backup.log 2>&1
#
# Env overrides:
#   BACKUP_DIR=backup            destination directory
#   RETENTION_DAYS=14            delete backups older than this (0 = keep all)
#   COMPOSE_PROJECT=adapta  compose project name (chroma volume prefix)
set -euo pipefail
...
COMPOSE_PROJECT="${COMPOSE_PROJECT:-adapta}"
DATE="$(date +%F)"
...
docker compose exec -T postgres pg_dump -U adapta adapta | gzip > "$BACKUP_DIR/adapta-$DATE.sql.gz"
...
if [ "$RETENTION_DAYS" -gt 0 ]; then
  ...
  find "$BACKUP_DIR" -maxdepth 1 -type f \
    \( -name 'adapta-*.sql.gz' -o -name 'chroma-*.tgz' -o -name 'artifacts-*.tgz' \) \
