...
2. **Dependencies live in `pyproject.toml` only (single source) and are baked into the image at build time.** Never `pip install` by hand — not on the host, not into a running container. The `dev` image stage already ships the `[dev]` toolchain, so `make dev` (= `docker compose -f docker-compose.yml -f docker-compose.dev.yml up -d`) yields a container where `make ci` runs immediately. **Bare `docker compose up` is production** (secure by default, §A4.5) — it does *not* ship the dev toolchain; use `make dev` for development. To add/change a dependency: edit `pyproject.toml`, then `make dev` (rebuilds). Multi-stage `Dockerfile` targets: `dev` (toolchain + bind-mounted source), `production` (lean, non-root, baked source), `worker` (`[training]` extras).
