...
Self-hosted, multi-user: registering creates an organization with you as its admin; teammates join an existing org via invites. A development admin (`admin@example.com` / `admin12345`) is seeded on an empty DB (`ADAPTA_SEED_DEFAULT_ADMIN`, on by default in compose). **Authoritative scope:** [docs/reference/PRODUCT_DEFINITION.md](docs/reference/PRODUCT_DEFINITION.md). The UI never calls RAG "training" — it asks *"give it knowledge"* (RAG) vs *"change how it behaves"* (fine-tuning).
...
1. **Do NOT rewrite the inference *engine* internals.** The llama-cpp integration in `adapta/core/inference.py` / `adapta/core/model_manager.py` and the real LoRA training loop in `adapta/training/trainer.py` are wrapped/called, not rewritten.
...
3. **Generated/derived artifacts are never hand-edited.** `adapta/models/generated/models.py` (regenerate via `make generate`); Alembic migrations are reviewed but autogenerated.
4. **No exception reaches a client as a raw string.** Never write `raise HTTPException(status_code=500, detail=str(e))`. Raise a typed `DomainError` from `adapta/domain/errors.py`; the boundary handler in `adapta/api/app.py` serializes it. `make check-leaks` enforces this in CI.
...
POST /v1/chat/completions                OpenAI-compatible (scoped adp_* key, model = endpoint slug);
...
| File | Purpose |
|---|---|
| `adapta/api/app.py` | FastAPI app factory; lifespan, CORS, correlation-ID middleware, global error handlers, router mounts |
| `adapta/domain/errors.py` | Typed `DomainError` taxonomy — the only exception types raised in services |
| `adapta/config.py` | All settings via pydantic-settings (DB URL, Redis URL, JWT secret, dirs, thresholds) |
...
| `adapta/core/model_catalog.py` | Base-model catalog SSOT (A3.3): HF id + GGUF + modality + mmproj per entry |
| `adapta/core/adapter_conversion.py` | PEFT→GGUF LoRA conversion (text + vision shims) |
| `adapta/services/adapters.py` | Adapter registry + eval threshold gate |
| `adapta/services/synthesis.py` | Document chunks → LLM-generated instruction pairs → JSONL |
| `adapta/worker/main.py` | Training worker process (BLPOP consumer, runs QLoRA, evaluates, registers adapter) |
| `adapta/api/v1/` | One router file per resource group |
...
- **Typed taxonomy** in `adapta/domain/errors.py`: `DomainError` + subclasses. Each carries `code`, HTTP `status`, safe `message`, and `internal_detail` that is **logged, never serialized**.
- **Single boundary translation** in `adapta/api/app.py`: `@app.exception_handler(DomainError)` and `@app.exception_handler(Exception)`. The catch-all returns a generic message + correlation ID only.
- **Correlation IDs**: middleware stamps `request.state.cid`, echoes `X-Correlation-ID`, logs it everywhere.
- **CI grep gate**: `make check-leaks` fails if `detail=str(e)` reappears anywhere in `adapta/`.
...
| Flaw | Was | Now |
|---|---|---|
| Mock orchestrator | `unified_router.py` returned `[Generated response using {model}]` | Deleted; `adapta/services/chat.py` is the only path |
| Error leakage | 63 `detail=str(e)` sites, 0 custom exceptions | `DomainError` taxonomy; `make check-leaks` CI gate |
| Placeholder embeddings | Random vectors in `memory_manager.py`, `hybrid_search.py` | Real sentence-transformers in `adapta/services/embeddings.py` |
| Dummy auth key | Hard-coded `"dummy"` string | bcrypt + JWT in `adapta/services/auth.py` |
...
| Dataset synthesis | Not implemented | `adapta/services/synthesis.py` + `POST /v1/projects/{id}/datasets/synthesize` |
